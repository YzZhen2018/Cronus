...
public class DialogItem {
    ...
    public void open(Player player, Dialog dialog) {
        player.openInventory(MenuBuilder.builder(Cronus.getInst())
                ...
                .rows(3)
                .items("#########", "$$$$%$$$$", "####@####")
                .put('%', ItemUtils.isNull(dialog.getItem()) ? null : MaterialControl.fromItem(dialog.getItem()).parseItem())
                ...
                .event(e -> {
                    if (e.getClickType() == ClickType.CLICK) {
                        ...
                        if (slot == 22) {
                            ...
                            dialog.setItem(ItemUtils.isNull(item) ? null : MaterialControl.fromItem(item).parseItem());
                            dialog.open(player);
                        }
                    }
                }).close(e -> {
